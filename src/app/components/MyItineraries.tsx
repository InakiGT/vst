...
export default function MyItineraries({ itineraries }: { itineraries: MyItinerary[] }) {
          ...
          itinerary => {
            ...
            return (
              <ul key={ id } className="grid grid-cols-7 gap-4 items-center py-3 px-5 text-xl border-b-borders border-b-2">
                ...
                <li className={`${  status === 'aceptada' ? 'text-green-600': 'text-red-400'}`}>{  status === 'aceptada' ? 'Aceptado' : ( status === 'rechazada' ? 'Rechazado' : 'Pendiente')  }</li>
                <li>
                  <Link href={`/itinerary/${  id }`} className="font-bold bg-main-blue rounded-sm py-1 px-8 cursor-pointer">Ver</Link>
                </li>
              </ul>
        )})
      }
